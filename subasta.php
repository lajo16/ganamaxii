...
// Forzar operación "subasta" si no viene en la URL
if (!isset($_GET['tipo_operacion']) || $_GET['tipo_operacion'] === '') {
  $_GET['tipo_operacion'] = 'subasta';
}
?>
<section class="hero py-5">
  <div class="container">
    <div class="p-4 p-md-5 rounded-4 overlay">
      <h1 class="display-6 fw-semibold mb-1">Participa en subastas de propiedades en todo el Perú</h1>
      <div class="small text-secondary">
        🔨 Subasta con respaldo legal y asesoría especializada.
      </div>
  ...
  <div class="container">
    ...
    <div class="row g-4">
      <?php
        ...
        // Total según los filtros actuales (incluye tipo_operacion = subasta)
        ...
        foreach($props as $p):
          ...
          // Texto y url para WhatsApp (menciona subasta)
          $waText = "Hola, quiero preguntar por esta propiedad en SUBASTA:\n" .
                    ($p['titulo'] ?? ('ID ' . $p['id'])) . "\n" .
                    ($p['distrito'] . ', ' . $p['provincia'] . ', ' . $p['departamento']) . "\n" .
                    "Precio/Referencia: $precioFmt\n" .
                    $detalleUrl;
...
<a class="btn btn-sm btn-ganamaxii px-4"
  ...
  Preguntar por esta propiedad
      ...
      <?php
        ...
        $qs = $_GET ?? [];
        unset($qs['page']);
        $self = basename($_SERVER['PHP_SELF']); // subasta.php
